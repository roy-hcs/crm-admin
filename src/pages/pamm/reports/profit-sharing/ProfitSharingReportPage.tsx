...
import { usePammReportProfitSharingList } from '@/api/hooks/pamm';
import { TableCell } from '@/components/ui/table';
import { ProfitSharingReportForm } from './ProfitSharingReportForm';
import { ProfitSharingReportTable } from './ProfitSharingReportTable';

export const ProfitSharingReportPage = () => {
  const [otherParams, setOtherParams] = useState<
    Omit<PammReportProfitSharingListParams, 'params' | keyof BasicParams>
  >({
    serverId: '',
    projectName: '',
    profitType: '',
  ...
  const [params, setParams] = useState<PammReportProfitSharingListParams['params']>({
    beginTime: '',
    endTime: '',
    agentName: '',
  });
  ...
  const { data: pammInvestReports, isLoading: pammInvestReportsLoading } =
    usePammReportProfitSharingList({
      pageSize,
      pageNum: pageNum + 1,
      orderByColumn,
      isAsc,
  ...
  return (
    <div>
      <h1 className="text-title">{t('PammProfitSharingReport.title')}</h1>
