...
import {
  useAddUserRole,
  useCheckUserRoleNameUnique,
  useEditUserRole,
  useUserRoleMenuTreeData,
...
export const AddEditUserRoleDialog = ({
  onSuccess,
  roleList,
  mode,
  open: openProp,
  ...
  const { data: roleMenuTreeData } = useUserRoleMenuTreeData(
  ...
  const { mutateAsync: checkRoleNameUnique, data: checkRoleNameRes } = useCheckUserRoleNameUnique();
  const { mutateAsync: add, isPending: isAddPending } = useAddUserRole();
  const { mutateAsync: edit, isPending: isEditPending } = useEditUserRole();
