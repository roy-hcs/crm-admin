...
import { useAddUserMenu, useEditUserMenu } from '@/api/hooks/system/system';
...
export const AddEditUserMenuDialog = ({
  mode,
  onSuccess,
  menuItem,
  parentOptions,
  ...
  const { mutateAsync: addMenu, isPending: isAddPending } = useAddUserMenu();
  const { mutateAsync: editMenu, isPending: isEditPending } = useEditUserMenu();
  ...
  const menuTypeOptions = useMemo(
    () => [
      { label: t('table.interLink'), value: 'I' },
      { label: t('table.outerLink'), value: 'O' },
